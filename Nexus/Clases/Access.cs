...
namespace Nexus.Clases
{
    public class Access
    {
        public int verify_access(string id_rol, int id_function)
        {
            ...
            try
            {
                ...
                sql.CommandText = "select dbo.verify_access(" + id_rol  +"," + id_function + ");";
                sql.Connection = con;
        ...
        public int verify_owner_template(string CWID,string id_template)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["BD_Base"].ConnectionString);
            int res = 0;
            string msg = "";
            try
            {
                ...
                con.Open();
                sql = new System.Data.SqlClient.SqlCommand();
                sql.CommandText = "select dbo.verify_owner_template('" + CWID + "','" + id_template + "');";
                sql.Connection = con;
                using (reader = sql.ExecuteReader())
                {
                    if (reader.Read())
